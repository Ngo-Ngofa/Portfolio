...
function contactBodyHTML(){
  ...
  if(to.indexOf("@") > -1){
    return `<form class="cform" id="cform" method="POST"
                  action="https://formsubmit.co/ajax/${encodeURIComponent(to)}">
  ...
  return `<div class="formsoon"><p>The form is not connected yet.
    See DEPLOY.md.</p></div>`;
}
...
function wireContact(){
  ...
  const say  = (cls, text) => { if(note){ note.className = "cnote " + cls; note.textContent = text; } };
  ...
  form.onsubmit = e => {
    ...
    fetch(form.action, {
      ...
      if(d && String(d.success) === "true"){
        form.reset();
        say("ok", SITE.contact.sentMessage || "Thanks. Message sent.");
      } else {
        throw new Error("failed");
      }
    }).catch(() => {
      say("bad", "That did not send. Please email " + SITE.brand.email + " instead.");
...
function renderApp(){
  ...
  <section class="block"><div class="w">
    ...
    <div class="split">
      <h2>${esc(a.summaryHeading || "What it solves")}</h2>
      ${colsHTML(a.problem)}
    </div>
  </div></section>
